import { TableSession, ITableSession, ISessionItem, ItemStatus } from '../models/TableSession.js';
...
export class TableSessionService {
  ...
  static async getActiveSession(tableId: string): Promise<ITableSession | null> {
  ...
  static async getAnalytics(restaurantId: string) {
    ...
    const totalRevenue = completedToday.reduce((sum, s) => sum + s.totalInPence, 0);
  ...
  private static calculateAverageSessionTime(sessions: ITableSession[]): number {
